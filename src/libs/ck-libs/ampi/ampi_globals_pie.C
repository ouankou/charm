...
void AMPI_Node_Setup(int numranks)
{
  ...
  SharedObject myexe = dlopen(binary_path.c_str(), RTLD_NOW|RTLD_LOCAL);
  ...
  pieglobalsdata.main_c.ptr = AMPI_Main_Get_C(myexe);
  pieglobalsdata.main_f.ptr = AMPI_Main_Get_F(myexe);
  pieglobalsdata.unpack.ptr = AMPI_FuncPtr_Unpack_Locate(myexe);

  CmiMemoryIsomallocContextActivate(CmiIsomallocContext{});
  CmiIsomallocContextEnableRecording(heap_context, 0);

  if (pieglobalsdata.main_c.ptr == nullptr && pieglobalsdata.main_f.ptr == nullptr)
...
template <typename T>
static void calc_funcptr_diff(T & funcptr, const void * objsource, const char * objend, const void * emptyheapstart, const char * allocation)
{
  auto ptr = (const char *)funcptr.ptr;
  if (ptr == nullptr)
  {
    funcptr.diff = null_funcptr_diff;
    return;
  }
  ...
  funcptr.diff = ptr - (const char *)objsource + (allocation - (const char *)emptyheapstart);
}
  ...
  if (localrank == 0)
  {
    for (const auto & obj : pieglobalsdata.objects)
    {
      ...
      for (const auto & item : obj.items)
      {
        ...
        char * end = src + len;
        calc_funcptr_diff(pieglobalsdata.main_c, src, end, heapstart, allocation);
        calc_funcptr_diff(pieglobalsdata.main_f, src, end, heapstart, allocation);
        calc_funcptr_diff(pieglobalsdata.unpack, src, end, heapstart, allocation);
      }
    }
  }
...
static inline int pieglobalsrun(int argc, char ** argv)
{
  const CthThread th = TCharm::get()->getThread();
  ...
  if (pieglobalsdata.unpack.diff != null_funcptr_diff)
  {
    auto newptr = (AMPI_FuncPtr_Unpack_t)(heapstart + pieglobalsdata.unpack.diff);
    AMPI_FuncPtr_Populate_Function(newptr);
  }

  if (pieglobalsdata.main_c.diff != null_funcptr_diff)
  {
    auto newptr = (ampi_maintype)(heapstart + pieglobalsdata.main_c.diff);
    return newptr(argc, argv);
  }
  else if (pieglobalsdata.main_f.diff != null_funcptr_diff)
  {
    auto newptr = (ampi_fmaintype)(heapstart + pieglobalsdata.main_f.diff);
    newptr();
    return 0;
  }
...
int main(int argc, char ** argv)
{
  ...
  return pieglobalsrun(argc, argv);
}
